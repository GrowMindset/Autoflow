...
const ConfirmDeleteDialog: React.FC<ConfirmDeleteProps> = ({ workflowName, onConfirm, onCancel, isDeleting }) => {
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="bg-white rounded-2xl shadow-[0_30px_80px_rgba(0,0,0,0.25)] border border-slate-100 w-full max-w-sm animate-in zoom-in-95 duration-200">
        <div className="p-6 pb-4">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h3 className="text-base font-black text-slate-800">Delete Workflow?</h3>
          ...
          <div className="bg-slate-50 rounded-xl px-4 py-3 border border-slate-100">
            <p className="text-sm text-slate-600 font-medium">
              ...
              <span className="font-black text-slate-800">"{workflowName}"</span>{' '}
              and all its data.
            </p>
          </div>
        </div>
        ...
        <div className="px-6 pb-6 flex items-center gap-3">
          <button
            ...
            onClick={onCancel}
            disabled={isDeleting}
            className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all disabled:opacity-50"
          >
            Cancel
          </button>
          <button
...
const WorkflowSidebar: React.FC<WorkflowSidebarProps> = ({
  ...
  return (
    <>
      ...
      <aside
        className={`h-screen bg-white border-r border-slate-200 transition-all duration-300 ease-in-out flex flex-col relative z-50 ${isCollapsed ? 'w-[68px]' : 'w-72'
          }`}
      >
                ...
                <div key={flow.id} className="relative group/item">
                  ...
                  >
                    ...
                    <div className={`flex-shrink-0 transition-all ${currentWorkflowId === flow.id
                      ...
                      : 'w-1.5 h-1.5 rounded-full bg-slate-200 group-hover:bg-slate-300'
                      }`} />
                  ...
                  {!isCollapsed && (
                    <button
                      ...
                      className="absolute right-2 top-1/2 -translate-y-1/2 w-6 h-6 rounded-lg bg-white border border-slate-200 text-slate-300 hover:text-red-500 hover:border-red-200 hover:bg-red-50 transition-all flex items-center justify-center opacity-0 group-hover/item:opacity-100 shadow-sm"
                    >
