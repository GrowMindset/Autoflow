...
const BaseNode: React.FC<NodeProps<WorkflowNodeData>> = ({ id, data, selected }) => {
  ...
  return (
    <div
      className={`px-3 py-2.5 rounded-xl border-2 transition-all shadow-none min-w-[150px] max-w-[150px] bg-white group/node relative 
        ${selected ? 'ring-2 ring-blue-500/10 border-blue-500' : 'border-slate-200'}
        ${data.status === 'RUNNING' ? 'border-emerald-600 ring-4 ring-emerald-400/30 bg-emerald-50 animate-pulse' : ''}
    ...
    >
      ...
      {data.status && (
        <div className="absolute -top-3 -right-3 p-1 rounded-full bg-white border border-slate-100 shadow-md z-20">
          {data.status === 'RUNNING' && <Loader2 size={12} className="text-emerald-600 animate-spin" />}
      ...
      {hasIncomplete && !data.status && (
        <div className="absolute -top-3 -left-3 p-1 rounded-full bg-amber-100 border border-amber-300 shadow-md z-20" title="Missing configuration">
