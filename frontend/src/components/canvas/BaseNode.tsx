...
const BaseNode: React.FC<NodeProps<WorkflowNodeData>> = ({ id, data, selected }) => {
  ...
  return (
    <div
      className={`relative px-3 py-2.5 rounded-xl border-2 transition-all shadow-sm min-w-[150px] max-w-[150px] bg-white dark:bg-slate-900 group/node 
        ${selected ? 'ring-2 ring-blue-500/20 dark:ring-blue-500/40 border-blue-500' : 'border-slate-200 dark:border-slate-800'}
        ${status === 'RUNNING' ? 'border-emerald-600 ring-4 ring-emerald-400/30 bg-emerald-50 dark:bg-emerald-900/40 animate-pulse' : ''}
        ${status === 'SUCCEEDED' ? 'border-emerald-500 ring-4 ring-emerald-500/10' : ''}
        ${status === 'FAILED' ? 'border-rose-500 ring-4 ring-rose-500/10' : ''}
      `}
      style={{ borderTop: `4px solid ${status === 'RUNNING' ? '#045e41' : accentColor}` }}
      title={hasIncomplete ? 'This node is missing required configuration' : ''}
    >
      {/* Execution Overlay */}
      {status === 'RUNNING' && (
        <div className="absolute inset-0 bg-white/60 dark:bg-slate-950/60 backdrop-blur-[1px] rounded-xl flex items-center justify-center z-[100] animate-in fade-in">
      ...
      {status && (
        <div className="absolute -top-3 -right-3 p-1 rounded-full bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-md z-20">
          {status === 'RUNNING' && <Loader2 size={12} className="text-emerald-600 animate-spin" />}
          {status === 'SUCCEEDED' && <Check size={12} className="text-emerald-500" strokeWidth={3} />}
          {status === 'FAILED' && <AlertCircle size={12} className="text-rose-500" />}
        </div>
      )}

      {/* Config Incomplete Warning */}
      {hasIncomplete && !status && (
        <div className="absolute -top-3 -left-3 p-1 rounded-full bg-amber-100 dark:bg-amber-900/50 border border-amber-300 dark:border-amber-700 shadow-md z-20" title="Missing configuration">
      ...
      <Handle
        ...
        className={`!w-1.5 !h-1.5 border border-white dark:border-slate-900 hover:!bg-blue-500 transition-all ${status === 'RUNNING' ? '!bg-emerald-500' : '!bg-slate-200 dark:!bg-slate-700'}`}
        style={{ left: -4 }}
      />

      <div className="flex flex-col gap-1 mt-0.5">
        <div className="flex items-center justify-between gap-3">
          <span className={`text-[8px] font-black uppercase tracking-widest`} style={{ color: status === 'RUNNING' ? '#059669' : accentColor }}>
            {data.category}
          </span>
          {data.is_dummy && <NodeBadge variant="neutral">Soon</NodeBadge>}
        </div>

        <h3 className={`text-xs font-bold leading-tight truncate ${status === 'RUNNING' ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-800 dark:text-slate-100'}`}>{data.label}</h3>

        <div className={`mt-1.5 flex items-center justify-between border-t ${status === 'RUNNING' ? 'border-emerald-200 dark:border-emerald-900/50' : 'border-slate-50 dark:border-slate-800'} pt-1.5 text-[8px] font-bold uppercase tracking-tighter ${status === 'RUNNING' ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-500'}`}>
          <span className="truncate max-w-[100px]">{data.type.replace('_', ' ')}</span>
          <div className={`w-1 h-1 rounded-full ${status === 'RUNNING' ? 'bg-emerald-500 animate-pulse' : data.is_dummy ? 'bg-slate-200 dark:bg-slate-700' : 'bg-green-400'}`} />
        </div>
      </div>
      ...
      {data.type === 'if_else' ? (
        <>
          <div className="absolute -right-1 top-[25%] flex items-center justify-end w-20 pointer-events-none">
            ...
            <Handle type="source" position={Position.Right} id="true" className={`!w-2 !h-2 border-2 border-white dark:border-slate-900 hover:!bg-emerald-500 transition-all pointer-events-auto ${status === 'RUNNING' ? '!bg-emerald-500' : '!bg-emerald-400'}`} />
          </div>
          <div className="absolute -right-1 top-[75%] flex items-center justify-end w-20 pointer-events-none">
            ...
            <Handle type="source" position={Position.Right} id="false" className={`!w-2 !h-2 border-2 border-white dark:border-slate-900 hover:!bg-rose-500 transition-all pointer-events-auto ${status === 'RUNNING' ? '!bg-emerald-500' : '!bg-rose-400'}`} />
          </div>
        </>
      ) : data.type === 'switch' ? (
        <div className="absolute -right-1 top-0 bottom-0 flex flex-col justify-center gap-4 py-2 pointer-events-none">
          {(data.config.cases || []).map((c: any, i: number) => (
            <div key={c.id || i} className="flex items-center justify-end w-32 translate-x-1">
              ...
              <Handle type="source" position={Position.Right} id={c.id || c.label || `case_${i}`} className={`!w-2 !h-2 border-2 border-white dark:border-slate-900 hover:!bg-blue-500 transition-all pointer-events-auto ${status === 'RUNNING' ? '!bg-emerald-500' : '!bg-blue-400'}`} />
            </div>
          ))}
          <div className="flex items-center justify-end w-32 translate-x-1">
            ...
            <Handle type="source" position={Position.Right} id="default" className={`!w-2 !h-2 border-2 border-white dark:border-slate-900 hover:!bg-slate-500 transition-all pointer-events-auto ${status === 'RUNNING' ? '!bg-emerald-500' : '!bg-slate-400'}`} />
          </div>
        </div>
      ) : (
        <Handle
          ...
          className={`!w-2 !h-2 border-2 border-white dark:border-slate-900 hover:!bg-blue-500 transition-all ${status === 'RUNNING' ? '!bg-emerald-500' : '!bg-slate-300 dark:!bg-slate-600'}`}
          style={{ right: -4 }}
        />
      )}
    </div>
